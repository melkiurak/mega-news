    ...
    return <div>
        <div className="flex flex-col gap-5" >
            ...
            <div className={`${postCardStyle} items-stretch justify-center phone:justify-between overflow-hidden rounded-xl p-5 -m-5 cursor-pointer`}>
                {posts.slice(slider, slider + visibleSlides).map((post) => (
                    <Link to={`/Post/${post.objectId}`} key={post.objectId}>
                        <div className={`${postItemStyle} gap-3 rounded-xl h-full p-2.5 shadow-[5px_0_20px_rgba(0,0,0,0.1),-5px_0_20px_rgba(0,0,0,0.1)] `}>
